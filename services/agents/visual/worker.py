...
def _legacy_template_scenes_enabled() -> bool:
    ...
    if raw is None or not raw.strip():
        return False  # OFF by default — legacy canned scenes must be explicitly opted in
...
def _extract_line_composition_counts(prompt: str) -> tuple[int, int]:
    straight = 0
    bendy = 0

    digit_straight = re.search(r"\b(\d{1,2})\s+straight\s+lines?\b", prompt)
    if digit_straight:
        straight = int(digit_straight.group(1))
    else:
        for token, value in COUNT_WORDS.items():
            if re.search(rf"\b{re.escape(token)}\s+straight\s+lines?\b", prompt):
                straight = value
                break

    digit_bendy = re.search(r"\b(\d{1,2})\s+(?:bendy|curvy|curved|wavy)\s+lines?\b", prompt)
    if digit_bendy:
        bendy = int(digit_bendy.group(1))
    else:
        for token, value in COUNT_WORDS.items():
            if re.search(rf"\b{re.escape(token)}\s+(?:bendy|curvy|curved|wavy)\s+lines?\b", prompt):
                bendy = value
                break

    if straight == 0 and bendy == 0 and _has_word(prompt, "line"):
        straight = 1

    return max(0, min(12, straight)), max(0, min(12, bendy))


def _wants_line_composition(prompt: str) -> bool:
    if "line" not in prompt and "lines" not in prompt:
        return False
    intent_tokens = ("draw", "paint", "sketch", "render", "show")
    if not any(_has_word(prompt, token) for token in intent_tokens):
        return False
    return any(token in prompt for token in ("straight", "bendy", "curvy", "curved", "wavy", "line"))


def _build_line_composition_strokes(prompt: str, mode: str) -> list[dict]:
    straight_count, bendy_count = _extract_line_composition_counts(prompt)
    total = straight_count + bendy_count
    if total <= 0:
        return []

    _, color_hex = _extract_color(prompt, default="#22d3ee")
    line_width = max(2, int(_size_hint(prompt, fallback=92) * 0.04))
    commands: list[dict] = []

    base_y = 0.2
    step_y = 0.12
    for idx in range(straight_count):
        y = min(0.9, base_y + idx * step_y)
        commands.append(
            {
                "op": "polyline",
                "id": f"straight_line_{idx+1}",
                "relative": True,
                "points": [[0.15, y, 0.12], [0.85, y, 0.12]],
                "color": color_hex,
                "line_width": line_width,
            }
        )

    bendy_base_index = straight_count
    for idx in range(bendy_count):
        y = min(0.9, base_y + (bendy_base_index + idx) * step_y)
        commands.append(
            {
                "op": "polyline",
                "id": f"bendy_line_{idx+1}",
                "relative": True,
                "points": [[0.12, y, 0.12], [0.32, y - 0.05, 0.14], [0.56, y + 0.06, 0.1], [0.84, y - 0.02, 0.13]],
                "color": color_hex,
                "line_width": line_width,
            }
        )
    ...
    return [
        {
            "stroke_id": "render-mode-line-composition",
            "kind": "setRenderMode",
            "params": {"mode": mode},
            "timing": {"start_ms": 0, "duration_ms": 80, "easing": "linear"},
        },
        {
            "stroke_id": "line-composition-script",
            "kind": "runScreenScript",
            "params": {"program": {"commands": commands}},
            "timing": {"start_ms": 40, "duration_ms": 1200, "easing": "linear"},
        },
        {
            "stroke_id": "line-composition-note",
            "kind": "annotateInsight",
            "params": {
                "text": (
                    f"Composed {straight_count} straight line(s) and {bendy_count} bendy line(s) "
                    "from prompt-level tool instructions."
                )
            },
            "timing": {"start_ms": 140, "duration_ms": 180, "easing": "linear"},
        },
    ]
...
# ---------------------------------------------------------------------------
# Scene builders: bouncing-balls and beyond
# ---------------------------------------------------------------------------
...
def generate_visual_strokes(prompt: str, context: Any | None = None) -> list[dict]:
    ...
    line_composition_scene = _wants_line_composition(p)
    ...
    if line_composition_scene:
        strokes.extend(_build_line_composition_strokes(prompt=p, mode=mode))
